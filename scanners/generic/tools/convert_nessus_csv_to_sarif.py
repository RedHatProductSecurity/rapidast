...
def convert_csv_to_sarif(csv_file):
    ...
    if is_file(csv_file):
        logging.debug(f"Reading input from '{csv_file}'")
    else:
        logging.debug("Reading input from STDIN")

    with open(csv_file, newline='') if is_file(csv_file) else sys.stdin as report:
        ...
        for row in reader:
            if row['Plugin ID'] == '19506':
                ...
                sarif_template["runs"][0]["tool"]["driver"]["name"] = nessus_info('Scanner edition used', row['Plugin Output'])
                sarif_template["runs"][0]["tool"]["driver"]["version"] = nessus_info('Nessus version', row['Plugin Output'])
                # Adding fullname to include policy
                sarif_template["runs"][0]["tool"]["driver"]["fullName"] = f"{nessus_info('Scanner edition used',row['Plugin Output'])} {nessus_info('Nessus version', row['Plugin Output'])} {nessus_info('Scan policy used', row['Plugin Output'])} Policy"
