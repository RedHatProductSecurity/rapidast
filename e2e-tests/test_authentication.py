...
class TestRapiDASTAuthentication(TestBase):
    def test_http_basic_authentication(self):
        ...
        assert "ZAP configured with HTTP Basic Authentication" in logs, \
        ...
        assert basic_auth_header_found, \
    ...
    def test_http_header_authentication(self):
        ...
        assert custom_header_found, \
            "Authorization header with exact custom value 'MySecretHeader' should be found in scan results"


def verify_specific_auth_header_value(
    report_data: Dict, header_name: str, expected_header_value: str
) -> bool:
    """
