...
def get_latest_main_tag() -> str:
    ...
    try:
        repo_path = os.getcwd()
        repo = Repo(repo_path)
        # Although we should filter tags just present in the 'main' branch, Konflux does not
        # provide branch information, so we just get the latest one
        tags = repo.tags
...
def custom_version_scheme(version: ScmVersion) -> str:
    ...
    return version.format_next_version(_version.guess_next_simple_semver, retain=_version.SEMVER_MINOR, fmt="{guessed}")
